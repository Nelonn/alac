...
static uint16_t Swap16NtoB(uint16_t inUInt16) {
...
static uint16_t Swap16BtoN(uint16_t inUInt16) {
#if TARGET_RT_LITTLE_ENDIAN
    return BSWAP16(inUInt16);
#else
    return inUInt16;
...
static uint32_t Swap32NtoB(uint32_t inUInt32) {
#if TARGET_RT_LITTLE_ENDIAN
...
static uint32_t Swap32BtoN(uint32_t inUInt32) {
#if TARGET_RT_LITTLE_ENDIAN
    return BSWAP32(inUInt32);
#else
    return inUInt32;
...
static uint64_t Swap64BtoN(uint64_t inUInt64) {
#if TARGET_RT_LITTLE_ENDIAN
...
static uint64_t Swap64NtoB(uint64_t inUInt64) {
#if TARGET_RT_LITTLE_ENDIAN
    return BSWAP64(inUInt64);
#else
    return inUInt64;
...
static float SwapFloat32BtoN(float in) {
#if TARGET_RT_LITTLE_ENDIAN
    union {
        float f;
        int32_t i;
...
static float SwapFloat32NtoB(float in) {
#if TARGET_RT_LITTLE_ENDIAN
    union {
        float f;
        int32_t i;
...
static double SwapFloat64BtoN(double in) {
#if TARGET_RT_LITTLE_ENDIAN
    union {
        double f;
        int64_t i;
...
static double SwapFloat64NtoB(double in) {
#if TARGET_RT_LITTLE_ENDIAN
    union {
        double f;
        int64_t i;
...
static void Swap16(uint16_t* inUInt16) {
    *inUInt16 = BSWAP16(*inUInt16);
}
static void Swap24(uint8_t* inUInt24) {
    uint8_t tempVal = inUInt24[0];
    inUInt24[0] = inUInt24[2];
    inUInt24[2] = tempVal;
}
static void Swap32(uint32_t* inUInt32) {
